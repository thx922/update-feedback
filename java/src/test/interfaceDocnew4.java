...
public class interfaceDocnew4 {
    ...
    private int times = 3;//参数递归层次取字段(参考)
    private int fieldCount =2;//参数递归层次取字段(循环字段)
...
//                String typeName =parameterTypes[i].getTypeName();
    ...
    public Map<String, Object> paramEntityAttr(String entityPrePath, Type ty, Map<String, Object> mapMethodParams) throws Exception {
        ...
        List<String> listAddParams3 = new ArrayList<String>();
        //有封装
...
//                        Field[] fd= annotation.annotationType().getFields();
//                        fd[0].getName();
...
//                        if("org.springframework.data.annotation.Transient".equals(annotationType)){
//                            listAttr.add("该参数不需要传");
//                        }else{
...
//                    listAddParams.add(map);
                    String attrs ="";
                    if (listAttr.size() ==0) {
                        attrs =  "<br/>字段 : "+field.getName() + " &nbsp;类型 : "+field.getType().getTypeName()+" &nbsp;描述 : " + listAttr ;
                    }else{
                        attrs =  "<br/>字段 : "+field.getName() + " &nbsp;类型 : "+field.getType().getTypeName() ;
                    }
//                    String attrs =  "<br/>"+field.getName() + ":" + listAttr ;
                    listAddParams2.add(attrs);

                    //字段深入判断
                    String oo =objectFieldFromObject(field, parms[1]);
                    fieldCount =2;
//                    listAddParams3.add(oo+"&nbsp;约束:"+listAttr+"<br/>");
                    if(listAttr.size()>0){
                        listAddParams3.add(oo+"<div style='width:70%;margin-left:22px;'><i style='margin-left:10px'>约束:"+listAttr+"</i></div>");
                    }else{
                        listAddParams3.add(oo+"<br/>");
                    }
                }
//                mapMethodParams.put("entityParamAttr", listAddParams);
                mapMethodParams.put(fieldsIndexStr + "EntityParamAttr", listAddParams2);
                String sd = listAddParams3.toString().substring(1,listAddParams3.toString().length()-1);
    ...
    public String example(String exampleType){
        String getTypePreValue ="";
        List<String> typePre = Arrays.asList("int", "long", "integer","float","double","String",
                "Boolean","list","map","Object","date","LocalDateTime","LocalDate");
        List<String> typePreValue = Arrays.asList("  1", "  1", "  1(或null)","  1.0"," 1.00","  你好",
                "  true ","list","  map","  Object","  2014-10-14 12:40:40 ","  2014-10-14 12:40:40 ","  2014-10-14 12:40:40  ");
        for (int i=0 ;i<typePre.size();i++) {
            String str = typePre.get(i);
            String lowerStr =exampleType.substring(0,1).toLowerCase()+exampleType.substring(1,exampleType.length());
            String upStr =exampleType.substring(0,1).toUpperCase()+exampleType.substring(1,exampleType.length());
            if (exampleType.contains(str) || lowerStr.equals(str) || upStr.equals(str)) {
                getTypePreValue ="'"+ typePreValue.get(i)+"'";
            }
            if(exampleType.startsWith("List") || exampleType.startsWith("Map")){
                getTypePreValue =exampleType.replaceAll("\\<","&lt;").replaceAll("\\>","&gt;");
            }
        }
        return getTypePreValue;
    }
    ...
    public String objectFieldFromObject( Field field, String typeName){
        ...
        if(clz != null && clz.getClassLoader() == null && !fieldSelf.equals(typeName)){//java对象
            ...
            String exampleStr = example( fielStr);
//            recursionField.append("<b>"+field.getName()+" : </b>"+fielStr.replaceAll("\\<","&lt;").replaceAll("\\>","&gt;")+"  "+exampleStr+" ,");
            recursionField.append("<b>"+field.getName()+" : </b> "+exampleStr+" ,");
        }else if((clz == null || clz.getClassLoader() != null)&& fieldSelf.equals(typeName) && !field.getType().isEnum()){
            recursionField.append("<b>"+field.getName()+" : </b>自身对象 ,");
        }else{                                          //非java对象
            ...
            for(Field fd: fds){
                ...
                if( !clz.isEnum() ){
                    if(fd.getType() == null || fd.getType().getClassLoader() != null){
                        ...
                        String exampleStr = example( fielStr);
                        recursionField.append("<p>" + fd.getName() + " :  "+exampleStr+ " ,</p>");
                    }
                } else if( clz.isEnum() ){
        ...
        for (int i = 0; i < listActName.size(); i++) {
            ...
            for (int j = 0; j < classList.size(); j++) {
                ...
                if (map.get("demo") != "" && map.get("demo") != null) {
                    ...
                    crulType = "No Example";
                    crulName = "<div class='com_con'>No Example</div>";
                }
...
//                                "<li><i></i><a href='javascript:;'>待续</a></li></ul>")
                ...
                if (map.get("paramTypeList") != null && !"".equals(map.get("paramTypeList")) && !"[]".equals(map.get("paramTypeList"))) {
                    ...
                    for (String param : params) {
                        ...
                        String attrLists = "";
                        StringBuffer bufAttrLists =new StringBuffer("");
