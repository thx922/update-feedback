package dnn.web2.admin.clouddisk.file;
...
public class FileAction2 implements IFileAction2 {

    private static final Logger LOGGER = LoggerFactory.getLogger(FileAction2.class);

    @Override
    public FileList list(FileListParameter fileListParameter) {
        LOGGER.info(JSON.toJSONString(fileListParameter));
        FileListParser fileListParser = new FileListParser(ClouddiskAction2.LOGIN_USER_TOKEN);
    ...
    public FileSearch search(String key, FileSearchParameter fileSearchParameter) {
        ...
        FileSearchParser fileSearchParser = new FileSearchParser(ClouddiskAction2.LOGIN_USER_TOKEN);
    ...
    public void readImgStream(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.isNotBlank(imgUrl)){
            AuthTokenParser authTokenParser = new AuthTokenParser(ClouddiskAction2.LOGIN_USER_TOKEN);
