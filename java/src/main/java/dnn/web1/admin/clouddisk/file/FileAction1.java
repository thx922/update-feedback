package dnn.web1.admin.clouddisk.file;
...
public class FileAction1 implements IFileAction1 {

    private static final Logger LOGGER = LoggerFactory.getLogger(FileAction1.class);

    @Override
    public FileList list(FileListParameter fileListParameter) {
        LOGGER.info(JSON.toJSONString(fileListParameter));
        FileListParser fileListParser = new FileListParser(ClouddiskAction1.LOGIN_USER_TOKEN);
    ...
    public FileSearch search(String key, FileSearchParameter fileSearchParameter) {
        ...
        FileSearchParser fileSearchParser = new FileSearchParser(ClouddiskAction1.LOGIN_USER_TOKEN);
    ...
    public void readImgStream(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.isNotBlank(imgUrl)){
            AuthTokenParser authTokenParser = new AuthTokenParser(ClouddiskAction1.LOGIN_USER_TOKEN);
